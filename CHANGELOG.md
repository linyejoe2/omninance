...
**Theme**: type-safe trading backend — the PostgreSQL store is rebuilt around Decimal money columns, enum statuses and an explicit daily-log lifecycle, with clean model / repository / engine / route separation. **Breaking**: the legacy `strategy` / `strategy_daily_log` / `trade_record` tables are dropped (fresh start) — rebuild the backend container and alembic creates the new schema on boot.

### Added
- `position` table — holdings as rows (weighted `average_cost`, `highest_price`, `trailing_stop_price`), replacing in-place JSONB `holdings_snapshot` mutation
- `order_record` table — request/fill breakdown (`req_*` vs `filled_*`), estimated `fee`/`tax`, `realized_pnl`, enum `status`/`action`/`sell_reason`; replaces `trade_record`
- alembic migrations (`migrations/`, baseline autogenerated by command); backend startup runs `upgrade head`
- `GET /api/strategies/{id}/positions` and `GET /api/order-records`; strategies gain `name` and `position_slots` (replaces `partition`)
- `src/repositories/` data-access layer — caller-owned sessions so multi-table fill updates commit atomically

### Changed
- All money flows through `Decimal`/`Numeric` — a fill updates order + position + the day's balance in one transaction; buy fee, sell fee/tax estimated at Taiwan rates
- `strategy_daily_log` reshaped: `status` lifecycle `signal-generated → executing → finalizing → ended`, `record_date` with DB-unique `(strategy_id, record_date)`, immutable signal/holding snapshots, per-day `errors` list
- Trading engine rewritten (`src/modules/trading_engine.py`): 15:30 next-day signal generation + trailing-stop updates, morning execution (stop-loss sells before buys), 15:00 mark-to-market settlement
- Strategy job names now kebab-case: `daily-strategies`, `finalize-daily-settlement`, `nightly-signal-generate` (ofelia.ini updated)
- Dashboard Strategy tab rewritten against the new API — positions table, daily-log status chips, order table with fee/tax/realized PnL
- chip-tracker `compute_signals` now honours request params (it silently used `setting.json` defaults, ignoring each strategy's settings) and emits `null` for NaN price/ATR instead of a fake `0.0`

### Removed
- The dangerous `cast(create_at, Date)` unique index on trade records — same-day duplicate protection is now an explicit query that lets `FAILED`/`CANCELLED` orders retry
- Dead code: `db_old.py`, `db_sync.py`, `src/test.py`, `pydantic_util.py`, the old god-module data access in `db.py`
