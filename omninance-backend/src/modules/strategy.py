...
from src.db import get_trade_records_by_ids, update_trade_record, get_last_unexecuted_daily_log, BuyObj, update_buy_obj, add_sell_obj, SellObj, update_sell_obj, engine, ensure_pydantic, Holding, is_symbol_traded_today, set_daily_log_to_executed, get_today_executed_daily_log, get_privous_daily_log
...
async def create_pending_signal_log(strategy_id: str, buy_symbol_list: list[str], snapshot: dict):
    ...
    prev_log = await get_last_daily_log(strategy_id)
...
async def execute_strategy(
    ...
    log = await get_last_unexecuted_daily_log(strategy_id)
    if not log:
    ...
    # 1. 先把目前持倉的代號做成一個集合 (Set)，搜尋效率最高
    current_symbols = {h["symbol"] for h in today_holdings}
    ...
    for buy_item in buy_list:
        # 這裡 buy_item 是字典，例如 {"symbol": "2330", ...}
        # 如果 BuyObj 當時轉成 dict 了，就要用 buy_item["symbol"]
        s_code = buy_item["symbol"] if isinstance(buy_item, dict) else buy_item.symbol
        
        # 檢查 A：目前持倉中是否已有 (避免重複持有)
        if s_code in current_symbols:
            continue
    ...
    await set_daily_log_to_executed(log.id)
...
async def poll_order_status(log_id: int, strategy_id: str, record_ids: list[int]) -> None:
    ...
    while pending_ids and attempt < max_attempts:
        ...
        try:
            ...
            records = await get_trade_records_by_ids(list(pending_ids))
            ...
            for rec in records:
                ...
                filled = order_info.filled_qty
                total  = order_info.total_qty
                price = order_info.avg_price
                
                # --- 1. 根據 Action 執行不同的 Side Effect (更新 Daily Log) ---
                if rec.action == "BUY":
                    # 買單：更新 buy_list 狀態
                    await update_buy_obj(log_id, BuyObj(
                        symbol=rec.symbol, 
                    ...
                    if filled > 0:
                        await update_sell_obj(log_id, SellObj(
                            symbol=rec.symbol,
                            price=price,
                            quantity=filled,
                ...
                if 0 < filled < total:
                    # 處理部分成交
                    await update_trade_record(
                        rec.id,
                        status="PARTIAL",
                        quantity=filled,
                        price=price,
                        result=f"Partially filled: {filled}/{total}"
                    )
                    # 注意：這時候不要把 rec.id 從 pending_ids 移除，因為它還沒結束
                    logger.info(f"[Poll] Order {ord_no} PARTIAL FILL: {filled} shares")
                # 完全成交
                ...
                elif order_info.is_failed:
                    await update_trade_record(
                        rec.id,
                        status="FAILED",
                        error=order_info.error_msg,
                    )
                    pending_ids.discard(rec.id)
                    logger.warning(f"[Poll] Order {ord_no} FAILED: {order_info.error_msg}")
    ...
    for remaining_id in pending_ids:
        ...
        await update_trade_record(remaining_id, status="TIMEOUT", error="Wait for filled timeout")
        
async def finalize_daily_settlement(strategy_id: str):
    # 1. 取得當前日誌與昨日日誌
    log = await get_today_executed_daily_log(strategy_id)
    if not log:
        logger.warning(f"[Daily Settlement] No log found for {get_date_tw_string()}, strategy_id: {strategy_id}")
        return
    privous_log = await get_privous_daily_log(log.id)
    if not privous_log:
        logger.warning(f"[Daily Settlement] No privous log found for {get_date_tw_string()}, strategy_id: {strategy_id}")
    ...
    # --- D. 存檔 ---
    await save_strategy_daily_log(log)
