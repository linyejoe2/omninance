...
def run_phase1(symbols: list[str], settings: dict):
    ...
    newest_date_cache: str | None = None
...
# ---------------------------------------------------------------------------
# Phase III — Portfolio Computation
# ---------------------------------------------------------------------------

def compute_portfolio(settings: dict) -> tuple[vbt.Portfolio, "pd.Series | None"]:
    """
    Load the pre-built matrices and run the vectorbt strategy portfolio.

    Returns (portfolio, benchmark_close).  Called by both run_phase3 (CLI)
    and the backtest API endpoint so the computation is never duplicated.
    """
...
def run_phase3(settings: dict) -> vbt.Portfolio:
    print("\n[Phase III] Vectorized Backtesting")
    pf, bm_close = compute_portfolio(settings)
    _report(pf, bm_close, settings["initial_capital"])
    return pf
